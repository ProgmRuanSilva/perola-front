...
import type { UpdatePessoaDTO } from "../../types/pessoa";
...
export default function PessoaEditar() {
  ...
  } = useForm<UpdatePessoaDTO>();

  useEffect(() => {
    document.title = "FASTFORWARD - Editar Usuário";
  }, []);

  useEffect(() => {
    async function carregar() {
      try {
        const pessoa = await apiFetch<any>(`/pessoas/${id}`);
        reset({
          nomePessoa: pessoa.nomePessoa,
          cpfPessoa: pessoa.cpfPessoa,
          contatoPessoa: pessoa.contatoPessoa,
          enderecoPessoa: pessoa.enderecoPessoa,
          tipoPessoa: pessoa.tipoPessoa,
        });
      } catch (err: any) {
        setErrorMsg(err.message || "Erro ao carregar dados da pessoa");
      } finally {
  ...
  const onSubmit = async (data: UpdatePessoaDTO) => {
    try {
      setErrorMsg(null);
      ...
      setErrorMsg(err.message || "Erro ao editar pessoa");
    }
  };
